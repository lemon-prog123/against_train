...
if __name__ == '__main__':
    ...
    #model.regularizationTerm(mode=1)
    for epoch in range(20):
        ...
        for i,data in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            output = model(inputs)
            loss = criterion(output, labels)+model.regularizationTerm(mode=1)
            loss.backward()
            optimizer.step()
        ...
        if epoch %1 == 0:
            ...
            save_checkpoint(save_path='checkpoint_regular',state={'epoch': epoch, 'state_dict': model.state_dict()},
                            is_best=is_best)
